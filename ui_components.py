...
class UIComponents:
    ...
    def create_menu_bar(self):
        ...
        # File operations
        self.file_menu.add_command(label="New", command=self.manager.file_ops.new_project)
        self.file_menu.add_command(label="Open...", command=self.manager.file_ops.open_file)
        self.file_menu.add_command(label="Save", command=self.manager.file_ops.save_file)
        self.file_menu.add_command(label="Save As...", command=self.manager.file_ops.save_file_as)
        ...
        # Edit operations
        self.edit_menu.add_command(label="Add Resource...", command=self.manager.task_ops.add_resource)
        self.edit_menu.add_command(label="Edit Resources...", command=self.manager.task_ops.edit_resources)
        self.edit_menu.add_separator()
        self.edit_menu.add_command(label="Project Settings...", command=self.manager.task_ops.edit_project_settings)
    ...
    def create_timeline_frame(self):
        ...
        self.manager.timeline_label_frame = tk.Frame(self.manager.timeline_frame, width=100)
        ...
        self.manager.h_scrollbar = ttk.Scrollbar(
            self.manager.main_frame, orient=tk.HORIZONTAL, command=self.sync_horizontal_scroll
        )
        ...
        self.manager.timeline_canvas.config(
            ...
            scrollregion=(0, 0, self.manager.cell_width * self.manager.days, self.manager.timeline_height)
        )

    def create_task_grid_frame(self):
        """Create the task grid canvas with both horizontal and vertical scrolling (Canvas 2)"""
        self.manager.task_frame = tk.Frame(self.manager.main_frame, height=self.manager.task_grid_height)
        self.manager.task_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 5))
        self.manager.task_frame.pack_propagate(False)  # Prevent frame from shrinking to content size
        ...
        self.manager.task_canvas.config(
            xscrollcommand=self.manager.h_scrollbar.set,
            scrollregion=(0, 0, self.manager.cell_width * self.manager.days, 
                        self.manager.max_tasks * self.manager.task_height)
        )

        # Bind mouse events for task manipulation
        self.manager.task_canvas.bind("<ButtonPress-1>", self.manager.task_ops.on_task_press)
        self.manager.task_canvas.bind("<B1-Motion>", self.manager.task_ops.on_task_drag)
        self.manager.task_canvas.bind("<ButtonRelease-1>", self.manager.task_ops.on_task_release)
        self.manager.task_canvas.bind("<Motion>", self.manager.task_ops.on_task_hover)
        self.manager.task_canvas.bind("<ButtonPress-3>", self.manager.task_ops.on_right_click)  
        ...
        self.manager.grid_resizer_frame = tk.Frame(self.manager.main_frame, height=5, 
                                        bg="gray", cursor="sb_v_double_arrow")
        ...
        self.manager.grid_resizer_frame.bind("<ButtonRelease-1>", self.on_resizer_release)
    ...
    def create_resource_grid_frame(self):
        ...
        # Create a fixed label column on the left
        self.manager.resource_label_frame = tk.Frame(self.manager.resource_frame, width=100)
        ...
        self.manager.resource_label_canvas.config(height=self.manager.resource_grid_height)
        
        # Connect scrollbar to canvas and sync labels with canvas
        self.manager.resource_vscrollbar.config(command=self.sync_resource_vertical_scroll)
    ...
    def create_context_menu(self):
        ...
        self.manager.context_menu.add_command(
            label="Edit Task Resources", command=self.manager.task_ops.edit_task_resources
        )
        self.manager.context_menu.add_separator()
        self.manager.context_menu.add_command(label="Delete Task", command=self.manager.task_ops.delete_task)
    ...
    def on_resizer_drag(self, event):
        ...
        available_height = self.manager.main_frame.winfo_height() - new_task_height - self.manager.timeline_height - 15
    ...
    def draw_task_grid(self):
        ...
        self.manager.task_canvas.config(scrollregion=(0, 0, canvas_width, canvas_height))
        self.manager.task_label_canvas.config(scrollregion=(0, 0, 100, canvas_height))

        # Draw the grid lines
        for i in range(self.manager.days + 1):
            x = i * self.manager.cell_width
            self.manager.task_canvas.create_line(
        ...
        for i in range(self.manager.max_tasks + 1):
            ...
            if i < self.manager.max_tasks:
                ...
                self.manager.task_label_canvas.create_text(
                    50, y + self.manager.task_height / 2, text=f"Row {i+1}", anchor="center"
                )
    ...
    def draw_resource_grid(self):
        ...
        self.manager.resource_canvas.config(scrollregion=(0, 0, canvas_width, canvas_height))
        self.manager.resource_label_canvas.config(scrollregion=(0, 0, 100, canvas_height))
