    ...
    - CI's "Publish to PyPI" workflow: the Jump to Version... dialog's two real-Tk tests had
      never actually been able to pass on GitHub Actions' headless runner (Xvfb reliably
      aborted the whole process on the first widget with text), silently blocking every release
      since they were added - they now skip under CI, where a real display isn't available
      (v0.1.25 through v0.1.27 were never published because of this).
    ### Removed
