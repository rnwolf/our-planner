...
class NetworkMenu:
    ...
    def _build_critical_path_dialog(self):
        """Build the Critical Path dialog shell: a centered, modal Toplevel
        with a title and an empty scrolled text area ready for results.

        Returns (dialog, content_frame, text_area).
        """
        ...
        return cp_dialog, frame, text_area

    def _render_critical_path_results(
        self, text_area, critical_path, path_length, network_analysis
    ):
        """Insert the critical-path analysis (or a no-path explanation) into
        text_area."""
        if critical_path:
            ...
            for task_id in critical_path:
                ...
                if task:
                    ...
                    text_area.insert(tk.END, f'Task {task_id}: {task["description"]}\n')
                    text_area.insert(tk.END, f'  Duration: {task["duration"]} days\n')
            ...
            text_area.insert(tk.END, '- No dependencies between the selected tasks\n')
            text_area.insert(tk.END, '- Circular dependencies in the selected tasks\n')
    ...
    def show_critical_path(self):
        ...
        try:
            ...
            cp_dialog, frame, text_area = self._build_critical_path_dialog()
            ...
            self._render_critical_path_results(
                text_area, critical_path, path_length, network_analysis
            )
