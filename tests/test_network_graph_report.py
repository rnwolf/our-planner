...
class TestBuildNetworkReportRows:
    ...
    def test_realistic_only_when_it_differs(self):
        model, ops = make_setup()
        cut = model.add_task(
            row=1, col=0, duration=5, description='Cut', resources={1: 1.0}
        )
        cut['realistic_duration'] = 10  # a real cut happened
        raw = model.add_task(
            row=2, col=0, duration=8, description='Raw', resources={1: 1.0}
        )  # realistic defaults to 8
    ...
    def test_chain_mapping(self):
        ...
        t3 = model.add_task(
            ...
            col=0,
            duration=2,
            description='On custom',
            resources={1: 1.0},
            chain_id=custom['id'],
        )
        t4 = model.add_task(
            row=4, col=0, duration=2, description='Loose', resources={1: 1.0}
        )
        rows = {r.name: r for r in ops.build_network_report_rows([t1, t2, t3, t4])}
    ...
    def test_buffer_types_pass_through(self):
        model, ops = make_setup()
        t = model.add_task(row=1, col=0, duration=3, description='PB', resources={})
...
class TestRenderedGraph:
    def test_resource_filter_uses_names(self):
        model, ops = make_setup()
        a = model.add_task(
            row=1, col=0, duration=2, description='A', resources={1: 1.0, 3: 1.0}
        )
        graph = render(ops, [a])
    ...
    def test_custom_chain_in_legend(self):
        ...
        graph = render(ops, [t])
        assert 'Integration stream' in {entry['label'] for entry in graph['legend']}

    def test_links_outside_selection_are_dropped(self):
        model, ops = make_setup()
        a = model.add_task(
            row=1, col=0, duration=2, description='A', resources={1: 1.0}
        )
        b = model.add_task(
            row=2,
            col=2,
        ...
        c = model.add_task(
            ...
            description='C',
            resources={1: 1.0},
            predecessors=[{'id': b['task_id'], 'type': 'FS', 'lag': 0}],
        )
